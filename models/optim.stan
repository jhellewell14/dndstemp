...
transformed data{
  ...
  matrix[61, 61] pimult;
  int p_shards = l / M; // number of positions per shard
  array [M, (p_shards * 61)] int xi;
  array [M, 61] real xr;
  ...
  for(i in 1:M){
    int j = 1 + (i - 1) * p_shards;
    int k = i * p_shards;
    for(g in 1:p_shards){
      xi[i, (1 + ((g - 1) * 61)):(((g - 1) * 61) + 61)] = X[j + (g - 1), 1:61];
    }
    xr[i, 1:61] = to_array_1d(pi_eq);
  }
...
model {
  target += sum(map_rect(f, beta, thetay, xr, xi));
...
generated quantities {
  // vector[M * p_shards] v = map_rect(f, beta, thetay, xr, xi);
  // real lik = sum(v);
  // matrix[61, 61] mutmat;
  // matrix[61, 61] V;
  // matrix[61, 61] V_inv;
